export function registerCustomLoaders(loader, loaders, callback) {
  if (!loaders.length) {
    callback();
    return;
  }
...
export function setSource(sources, loader, packageId, name, originalSource, language, callback) {
  let implementation;
...
export function getSource(sources, loader, name, callback) {
