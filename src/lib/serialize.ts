...
function modulesToData(modules, options) {
  ...
  const sourcePromises = data.components
    ...
    .map((component) => fs.readFile(component.path, 'utf-8')
      .then((source) => {
        ...
        return fs.readFile(component.tests, 'utf-8')
          .then((specs) => {
...
function produceLoader(data) {
  return Promise.resolve(`
// File generated by esbuild-plugin-noflo on ${new Date().toISOString()}
const baseLoader = require('esbuild-plugin-noflo/dist/loader.js');
...
export default serializeLoader;
